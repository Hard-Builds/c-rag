...
All configuration lives in `.env` (read by `app/core/config.py` via `pydantic-settings`). Required at startup:
...
START → upsert_thread → should_use_rag
          ├─(use_rag=True)→ context_retriever → chat_bot
...
| `app/bot/nodes/` | Individual graph nodes |
